...
async function setupUser (user) {
  ...
  await exec(`su -c 'cd && touch .ssh/authorized_keys && chmod 600 .ssh/authorized_keys' ${user.name}`)

  const keyPrefix = 'no-port-forwarding,no-X11-forwarding,no-agent-forwarding,no-pty '
  ...
  await exec(`chown -R ${user.name}:${user.name} /home/${user.name}/git-shell-commands `)
