...
export default function Login ({ setLoginStatus }) {
  ...
  async function handleSubmit (e) {
    ...
    if (signUp) {
      try {
        const result = await gitlubService.registerUser({
          username,
          password,
          sshKey
        })
  ...
  return (
    <div className='max-w-screen-xl px-4 py-16 mx-auto mt-6 sm:px-6 lg:px-8'>
      <div className='max-w-lg mx-auto space-y-4'>
        ...
        <p className='max-w-md mx-auto mt-4 text-3xl text-center text-gray-900'>
          {signUp ? 'Welcome to GitLub' : 'Sign in to GitLub'}
        </p>

        <form
          className='p-8 mt-6 mb-0 space-y-4 rounded-lg bg-gray-50 border border-gray-200'
        ...
        >
          ...
          <button
            type='submit'
            className='block mt-14 w-full px-5 py-3 text-lg font-medium text-white bg-green-600 hover:bg-green-700 rounded-lg'
          >
